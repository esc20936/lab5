...
// mpicc -g -Wall -o mpi_trap4 mpi_trap4.c
 ...
 */void Build_mpi_type(double *a_p, double *b_p, int *n_p, MPI_Datatype *input_mpi_t_p)
{
    ...
    // Define los desplazamientos y tipos en el nuevo orden {b, n, a}
    ...
    // Crea el nuevo tipo derivado MPI
    ...
    int *n_p /* out */)
{
    ...
    Build_mpi_type(b_p, n_p, a_p, &input_mpi_t); // Cambio de orden en Build_mpi_type

    if (my_rank == 0)
    {
        printf("Enter a, b, and n\n");
        scanf("%lf %lf %d", b_p, n_p, a_p); // Cambio de orden en scanf
    }

    // Utiliza el tipo derivado de MPI para transmitir la estructura InputData.
    MPI_Bcast(b_p, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Bcast(n_p, 1, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Bcast(a_p, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD); // Cambio de orden en las llamadas a MPI_Bcast
